...
<body class="min-h-screen flex bg-white font-sans">
    ...
    <div class="flex-1 flex flex-col justify-center px-8 md:px-24 lg:px-32 relative animate-fade-in">
        <a href="<?php echo e(url('/')); ?>" class="absolute top-12 left-8 md:left-24 lg:left-32 flex items-center gap-2 text-slate-400 hover:text-indigo-600 transition-colors font-bold text-sm">
        ...
        <div class="max-w-md w-full mx-auto animate-slide-up">
            ...
            <?php if(session('status')): ?>
                <div class="mb-6 p-4 bg-emerald-50 border border-emerald-200 rounded-2xl">
                    <p class="text-sm font-medium text-emerald-800"><?php echo e(session('status')); ?></p>
                </div>
            <?php endif; ?>

            <form method="POST" action="<?php echo e(route('login')); ?><?php echo e(request()->has('tenant') ? '?tenant=' . request('tenant') : ''); ?>" class="space-y-6">
                <?php echo csrf_field(); ?>
                
                <?php if(request()->has('tenant')): ?>
                    <input type="hidden" name="tenant" value="<?php echo e(request('tenant')); ?>">
                <?php endif; ?>
            ...
            <div class="mt-12 pt-8 border-t border-slate-100 text-center">
                <p class="text-slate-500 font-medium">
                    ...
                    <a href="<?php echo e(route('tenant.register')); ?>" class="text-blue-600 font-black hover:underline">Create one for free</a>
                </p>
            </div>
        </div>
    </div>
...
<?php /**PATH C:\lovehills\404 manager 1\resources\views/auth/login-react.blade.php ENDPATH**/ ?>